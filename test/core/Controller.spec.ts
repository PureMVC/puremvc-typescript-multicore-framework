...
describe("ControllerTest", () => {
  ...
  test("testGetInstance", () => {
    // Test Factory Method
    const controller: IController = Controller.getInstance(
      "ControllerTestKey1",
  ...
  test("testConstructorWithExistingKey", () => {
    ...
    const controller1: IController = Controller.getInstance(
      key,
      (key: string) => new Controller(key),
    );
    ...
    expect(() => {
      ...
      Error("Controller instance for this Multiton key already constructed!"),
    );
  });
