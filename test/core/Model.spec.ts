...
describe("ModelTest", () => {
  ...
  test("testGetInstance", () => {
    // Test Factory Method
    const model: IModel = Model.getInstance(
      "ModelTestKey1",
      (key: string) => new Model(key),
    );
  ...
  test("testRegisterAndRetrieveProxy", () => {
    ...
    const proxy: IProxy | null = model.retrieveProxy("colors");
  ...
  test("testRegisterAndRemoveProxy", () => {
    ...
    // remove the proxy
    const removedProxy: IProxy | null = model.removeProxy("sizes");
    ...
    const proxy1 = model.retrieveProxy("sizes");
  ...
  test("testHasProxy", () => {
    ...
    const proxy: IProxy = new Proxy("aces", [
      "clubs",
      "spades",
      "hearts",
      "diamonds",
  ...
  test("testOnRegisterAndOnRemove", () => {
    ...
    const proxy: IProxy = new ModelTestProxy();
    model.registerProxy(proxy);
